...
public class GamesController : ControllerBase
{
    ...
    public async Task<IActionResult> Create([FromBody] CreateGameRequest request)
    {
        ...
        var maxTurns = request.MaxTurns ?? 200;
        var creatorLang = User.FindFirstValue("preferred_language") ?? "en";

        foreach (var invitedUser in invitedUsers.Where(u => u.Id != userId).DistinctBy(u => u.Id))
        {
            var lang = invitedUser.PreferredLanguage ?? "en";
            var isAdmin = adminEmails.Contains(invitedUser.Email);
            var (subject, html) = EmailTemplates.GameInviteRegistered(game.Name, gameTypeId, maxTurns, creatorName, appUrl, isAdmin, lang);
        ...
        foreach (var newEmail in playerEmails.Select(e => e.Trim().ToLower()).Distinct())
        {
            ...
            // New users get the creator's language
            var (subject, html) = EmailTemplates.GameInviteUnregistered(game.Name, gameTypeId, maxTurns, creatorName, appUrl, isAdmin, creatorLang);
